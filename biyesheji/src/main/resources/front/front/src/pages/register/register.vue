<template>
	...
	<div class="container" :style='{"minHeight":"100vh","alignItems":"center","background":"url(http://codegen.caihongy.cn/20230918/e4c3de45e8454be08468ba32acc9108f.jpg)","display":"flex","width":"100%","backgroundSize":"cover","backgroundPosition":"center center","backgroundRepeat":"no-repeat","justifyContent":"center"}'>
		<el-form class='rgs-form' v-if="pageFlag=='register'" :style='{"width":"30vw","padding":"0px","margin":"0 0 0 50vw","borderRadius":"0px","background":"#fff","height":"auto"}' ref="registerForm" :model="registerForm" :rules="rules">
			...
			<div v-if="true" :style='{"width":"80%","margin":"20px auto 10px","lineHeight":"44px","fontSize":"28px","color":"#333","textAlign":"left"}'>基于spring boot的民宿管理系统的设计与实现注册</p></div>
			...
			<el-form-item :style='{"width":"auto","padding":"0","margin":"0 auto 15px 10%","background":"none","height":"auto"}' v-if="tableName=='minsushangjia'" prop="minsuhao">
				<div v-if="true" :style='{"width":"100%","lineHeight":"36px","fontSize":"14px","color":"#333"}' :class="changeRules('minsuhao')?'required':''">民宿号：</div>
				<el-input v-model="registerForm.minsuhao"  placeholder="请输入民宿号" />
			</el-form-item>
			<el-form-item :style='{"width":"auto","padding":"0","margin":"0 auto 15px 10%","background":"none","height":"auto"}' v-if="tableName=='minsushangjia'" prop="mima">
				<div v-if="true" :style='{"width":"100%","lineHeight":"36px","fontSize":"14px","color":"#333"}' :class="changeRules('mima')?'required':''">密码：</div>
				<el-input v-model="registerForm.mima" type="password" placeholder="请输入密码" />
			</el-form-item>
			<el-form-item :style='{"width":"auto","padding":"0","margin":"0 auto 15px 10%","background":"none","height":"auto"}' v-if="tableName=='minsushangjia'" prop="mima2">
				<div v-if="true" :style='{"width":"100%","lineHeight":"36px","fontSize":"14px","color":"#333"}' :class="changeRules('mima')?'required':''">确认密码：</div>
				<el-input v-model="registerForm.mima2" type="password" placeholder="请再次输入密码" />
			</el-form-item>
			<el-form-item :style='{"width":"auto","padding":"0","margin":"0 auto 15px 10%","background":"none","height":"auto"}' v-if="tableName=='minsushangjia'" prop="minsumingcheng">
				<div v-if="true" :style='{"width":"100%","lineHeight":"36px","fontSize":"14px","color":"#333"}' :class="changeRules('minsumingcheng')?'required':''">民宿名称：</div>
				<el-input v-model="registerForm.minsumingcheng"  placeholder="请输入民宿名称" />
			</el-form-item>
			<el-form-item :style='{"width":"auto","padding":"0","margin":"0 auto 15px 10%","background":"none","height":"auto"}' v-if="tableName=='minsushangjia'" prop="minsudizhi">
				<div v-if="true" :style='{"width":"100%","lineHeight":"36px","fontSize":"14px","color":"#333"}' :class="changeRules('minsudizhi')?'required':''">民宿地址：</div>
				<el-input v-model="registerForm.minsudizhi"  placeholder="请输入民宿地址" />
			</el-form-item>
			<el-form-item :style='{"width":"auto","padding":"0","margin":"0 auto 15px 10%","background":"none","height":"auto"}' v-if="tableName=='minsushangjia'" prop="lianxidianhua">
			...
			<el-form-item :style='{"width":"auto","padding":"0","margin":"0 auto 15px 10%","background":"none","height":"auto"}' v-if="tableName=='minsushangjia'" prop="yingyezhizhao">
	...
	mounted() {
		if(this.$route.query.pageFlag=='register'){
			...
			if ('minsushangjia' == this.tableName) {
				this.requiredRules.minsuhao = [{ required: true, message: '请输入民宿号', trigger: 'blur' }]
			}
			if ('minsushangjia' == this.tableName) {
				this.requiredRules.mima = [{ required: true, message: '请输入密码', trigger: 'blur' }]
			}
			if ('minsushangjia' == this.tableName) {
				this.requiredRules.minsumingcheng = [{ required: true, message: '请输入民宿名称', trigger: 'blur' }]
			}
			if ('minsushangjia' == this.tableName) {
				this.requiredRules.minsudizhi = [{ required: true, message: '请输入民宿地址', trigger: 'blur' }]
			}
		}
	},
    created() {
          ...
          if (valid) {
            var url=this.tableName+"/register";
               ...
               if(`yonghu` == this.tableName && this.registerForm.mima!=this.registerForm.mima2) {
				...
				if((!this.registerForm.shoujihaoma) && `yonghu` == this.tableName){
				...
				if((!this.registerForm.minsuhao) && `minsushangjia` == this.tableName){
					this.$message.error(`民宿号不能为空`);
					return
				}
               if(`minsushangjia` == this.tableName && this.registerForm.mima!=this.registerForm.mima2) {
                this.$message.error(`两次密码输入不一致`);
                return
               }
				if((!this.registerForm.mima) && `minsushangjia` == this.tableName){
					this.$message.error(`密码不能为空`);
					return
				}
				if((!this.registerForm.minsumingcheng) && `minsushangjia` == this.tableName){
					this.$message.error(`民宿名称不能为空`);
					return
				}
				if((!this.registerForm.minsudizhi) && `minsushangjia` == this.tableName){
					this.$message.error(`民宿地址不能为空`);
					return
				}
					if(`minsushangjia` == this.tableName && this.registerForm.lianxidianhua &&(!this.$validate.isMobile2(this.registerForm.lianxidianhua))){
