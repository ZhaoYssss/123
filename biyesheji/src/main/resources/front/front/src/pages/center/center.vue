...
<div class="center-preview" :style='{"padding":"0 7%","margin":"20px auto 40px","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","position":"relative","justifyContent":"space-between"}'>
	...
	<div class="info" :style='{"padding":"10px","boxShadow":"0 0px 0px rgba(0, 0, 0, 0.3)","margin":"20px 3% 0 0","borderColor":"#eee","borderRadius":"0px","background":"#ffffff","borderWidth":"4px 2px 1px","width":"25%","borderStyle":"solid","height":"auto"}'>
		...
		<div :style='{"borderColor":"#efefef","margin":"0 auto","borderWidth":"0 0 1px 0","width":"90%","fontSize":0,"borderStyle":"solid","height":"auto"}' v-if="userTableName=='minsushangjia'">
			<el-image :style='{"margin":"10px auto","borderColor":"#efefef","objectFit":"cover","borderRadius":"100%","borderWidth":"0 0 1px 0","display":"block","width":"80px","borderStyle":"solid","height":"80px"}' :src="sessionForm.yingyezhizhao?baseUrl + sessionForm.yingyezhizhao:require('@/assets/avator.png')" fit="cover"></el-image>
		</div>
		...
		<div :style='{"padding":"0 20px","borderColor":"#efefef","margin":"0 auto","borderWidth":"0 0 1px 0","display":"flex","width":"90%","lineHeight":"40px","borderStyle":"solid","height":"auto"}' v-if="userTableName=='minsushangjia'">
			<span class="icon iconfont icon-shouye-zhihui" :style='{"color":"#333","padding":"0 5px","fontSize":"14px"}'></span>
			<div :style='{"color":"#333","fontSize":"14px"}'>民宿号</div>
			<div :style='{"fontSize":"14px","color":"#333","textAlign":"right","flex":1}'>{{sessionForm.minsuhao}}</div>
		</div>
		<div :style='{"padding":"0 20px","borderColor":"#efefef","margin":"0 auto","borderWidth":"0 0 1px 0","display":"flex","width":"90%","lineHeight":"40px","borderStyle":"solid","height":"auto"}' v-if="userTableName=='minsushangjia'">
			<span class="icon iconfont icon-shouye-zhihui" :style='{"color":"#333","padding":"0 5px","fontSize":"14px"}'></span>
			<div :style='{"color":"#333","fontSize":"14px"}'>民宿名称</div>
			<div :style='{"fontSize":"14px","color":"#333","textAlign":"right","flex":1}'>{{sessionForm.minsumingcheng}}</div>
		</div>
		<div :style='{"padding":"0 20px","borderColor":"#efefef","margin":"0 auto","borderWidth":"0 0 1px 0","display":"flex","width":"90%","lineHeight":"40px","borderStyle":"solid","height":"auto"}' v-if="userTableName=='minsushangjia'">
			<span class="icon iconfont icon-shouye-zhihui" :style='{"color":"#333","padding":"0 5px","fontSize":"14px"}'></span>
			<div :style='{"color":"#333","fontSize":"14px"}'>民宿地址</div>
			<div :style='{"fontSize":"14px","color":"#333","textAlign":"right","flex":1}'>{{sessionForm.minsudizhi}}</div>
		</div>
		<div :style='{"padding":"0 20px","borderColor":"#efefef","margin":"0 auto","borderWidth":"0 0 1px 0","display":"flex","width":"90%","lineHeight":"40px","borderStyle":"solid","height":"auto"}' v-if="userTableName=='minsushangjia'">
			<span class="icon iconfont icon-shouye-zhihui" :style='{"color":"#333","padding":"0 5px","fontSize":"14px"}'></span>
    ...
    <el-tabs tab-position="left" :style='{"width":"72%","margin":"20px 0 0","flexWrap":"wrap","background":"#fff","flex":"1","display":"flex"}' @tab-click="handleClick">
      <el-tab-pane label="个人中心">
        <el-form class="center-preview-pv" ref="sessionForm" :model="sessionForm" :rules="rules" label-width="180px">
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"#fff"}' v-if="userTableName=='minsushangjia'" label="民宿号" prop="minsuhao">
            <el-input v-model="sessionForm.minsuhao" placeholder="民宿号" readonly></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"#fff"}' v-if="userTableName=='minsushangjia'" label="民宿名称" prop="minsumingcheng">
            <el-input v-model="sessionForm.minsumingcheng" placeholder="民宿名称" ></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"#fff"}' v-if="userTableName=='minsushangjia'" label="民宿地址" prop="minsudizhi">
            <el-input v-model="sessionForm.minsudizhi" placeholder="民宿地址" ></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"#fff"}' v-if="userTableName=='minsushangjia'" label="联系电话" prop="lianxidianhua">
            <el-input v-model="sessionForm.lianxidianhua" placeholder="联系电话" ></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"#fff"}' v-if="userTableName=='minsushangjia'" label="营业执照" prop="yingyezhizhao">
			<file-upload
			tip="点击上传营业执照"
			action="file/upload"
			:limit="1"
			...
			@change="minsushangjiayingyezhizhaoHandleAvatarSuccess"
			></file-upload>
          </el-form-item>
...
<script>
  ...
  export default {
    ...
    created() {
		...
		if ('minsushangjia' == this.userTableName) {
			this.$set(this.sessionForm, 'minsuhao', null);
		}
		if ('minsushangjia' == this.userTableName) {
			this.$set(this.sessionForm, 'mima', null);
		}
		if ('minsushangjia' == this.userTableName) {
			this.$set(this.sessionForm, 'minsumingcheng', null);
		}
		if ('minsushangjia' == this.userTableName) {
			this.$set(this.sessionForm, 'minsudizhi', null);
		}
		if ('minsushangjia' == this.userTableName) {
		...
		if ('yonghu' == this.userTableName&&this.rules['shoujihaoma']){
			this.rules['shoujihaoma'].push({ required: true, message: '请输入手机号码', trigger: 'blur' })
		}else if('yonghu' == this.userTableName&&!this.rules['shoujihaoma']) {
			this.$set(this.rules, 'shoujihaoma', [{ required: true, message: '请输入手机号码', trigger: 'blur' }]);
		}
		if ('minsushangjia' == this.userTableName&&this.rules['minsuhao']){
			this.rules['minsuhao'].push({ required: true, message: '请输入民宿号', trigger: 'blur' })
		}else if('minsushangjia' == this.userTableName&&!this.rules['minsuhao']) {
			this.$set(this.rules, 'minsuhao', [{ required: true, message: '请输入民宿号', trigger: 'blur' }]);
		}
		if ('minsushangjia' == this.userTableName&&this.rules['mima']){
			this.rules['mima'].push({ required: true, message: '请输入密码', trigger: 'blur' })
		}else if('minsushangjia' == this.userTableName&&!this.rules['mima']) {
			this.$set(this.rules, 'mima', [{ required: true, message: '请输入密码', trigger: 'blur' }]);
		}
		if ('minsushangjia' == this.userTableName&&this.rules['minsumingcheng']){
			this.rules['minsumingcheng'].push({ required: true, message: '请输入民宿名称', trigger: 'blur' })
		}else if('minsushangjia' == this.userTableName&&!this.rules['minsumingcheng']) {
			this.$set(this.rules, 'minsumingcheng', [{ required: true, message: '请输入民宿名称', trigger: 'blur' }]);
		}
		if ('minsushangjia' == this.userTableName&&this.rules['minsudizhi']){
			this.rules['minsudizhi'].push({ required: true, message: '请输入民宿地址', trigger: 'blur' })
		}else if('minsushangjia' == this.userTableName&&!this.rules['minsudizhi']) {
			this.$set(this.rules, 'minsudizhi', [{ required: true, message: '请输入民宿地址', trigger: 'blur' }]);
		}
		if ('minsushangjia' == this.userTableName) {
    ...
    methods: {
      ...
      onSubmit(formName) {
		...
		if(`minsushangjia` == this.userTableName && this.sessionForm.yingyezhizhao!=null){
      ...
      minsushangjiayingyezhizhaoHandleAvatarSuccess(fileUrls) {
