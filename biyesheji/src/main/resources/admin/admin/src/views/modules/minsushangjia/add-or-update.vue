	...
	<div class="addEdit-block" :style='{"padding":"30px","fontSize":"16px","color":"#666","background":"none"}'>
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="upload" v-if="type!='info' && !ro.yingyezhizhao" label="营业执照" prop="yingyezhizhao">
					<file-upload
						tip="点击上传营业执照"
						action="file/upload"
						:limit="3"
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="upload" v-else-if="ruleForm.yingyezhizhao" label="营业执照" prop="yingyezhizhao">
...
export default {
	data() {
		...
		return {
			...
			rules: {
				minsuhao: [
					{ required: true, message: '民宿号不能为空', trigger: 'blur' },
				],
				mima: [
				...
				minsudizhi: [
					{ required: true, message: '民宿地址不能为空', trigger: 'blur' },
				],
				lianxidianhua: [
				...
				this.$http({ 
					...
					if (data && data.code === 0) { 
						if(data.data.total>=crossoptnum) {
							...
							this.$http({
								url: `minsushangjia/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.minsushangjiaCrossAddOrUpdateFlag = false;
											this.parent.search();
